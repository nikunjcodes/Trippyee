...
const Cities: React.FC = () => {
    ...
    const fetchCities = async () => {
      try {
        ...
        const response = await axios.get<City[]>(
          `https://trippyee.onrender.com/api/search?q=${query}`
        );
    ...
    if (expandedCard === cityName) {
      ...
      try {
        const response = await axios.get<Place[]>(
          `https://trippyee.onrender.com/api/places/${encodeURIComponent(
            cityName
          )}`
        );
  ...
  const cleanPlaceDescription = (description: string) => {
    return description ? description.replace(/[\[\]\"\'0-9.]/g, "") : "";
  };
  ...
  const handleCreateItinerary = (city: City) => {
    if (selectedPlaces.length === 0) {
      toast({
        ...
        description:
          "Please select at least one place to visit before creating your itinerary.",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-100 to-blue-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {cities.length === 0 ? (
          <div className="text-2xl font-semibold text-center text-green-800">
          ...
          <div className="space-y-8">
            {cities.map((city) => (
              <motion.div
                ...
                className={
                  expandedCard && expandedCard !== city.City
                    ? "filter blur-sm"
                    : ""
                }
              >
                  ...
                  <div className="md:w-2/3 flex flex-col">
                    ...
                    <CardContent className="p-6 flex-grow">
                      ...
                      <p className="text-sm text-gray-600 mb-4 flex items-center">
                        ...
                        {city.Min_no_of_days} - {city.Max_no_of_days} days
                      </p>
            ...
            <Card className="w-full max-w-4xl overflow-auto max-h-[90vh] bg-gradient-to-r from-brown-600 to-brown-800 rounded-lg shadow-xl">
              <CardHeader className="bg-blue-500 text-white sticky top-0 z-10 py-4 px-6">
                ...
                <CardDescription className="text-green-100 flex items-center">
                  ...
                  {typeof cities.find((c) => c.City === expandedCard)
                    ?.Ratings === "number"
                    ? cities
                        .find((c) => c.City === expandedCard)
                        ?.Ratings.toFixed(1)
                    : "N/A"}
                </CardDescription>
                <Button
                ...
                {places.map((place) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      >
                        <span className="ml-2">
                          {cleanPlaceDescription(place.Place)}
                        </span>
                      </label>
                      <p className="text-sm text-gray-600 ml-2">
                        <StarIcon className="w-4 h-4 inline-block mr-1 fill-current text-yellow-400" />
                        {place.Ratings ? place.Ratings.toFixed(1) : "4.0"} |{" "}
                        {place.Distance}
                      </p>
                      <p className="text-gray-700 ml-2">
                        {cleanPlaceDescription(place.Place_Desc)}
                      </p>
                    </div>
                  </div>
                ))}
              ...
              <CardFooter className="px-6 py-4 flex flex-col items-start space-y-4 bg-green-700 rounded-b-lg">
                <p className="text-lg font-semibold text-white">
                  Estimated trip duration: {estimatedDays} days
                </p>
                <Button
                  onClick={() =>
                    handleCreateItinerary(
                      cities.find((c) => c.City === expandedCard)!
                    )
                  }
