...
const MatchingCities: React.FC = () => {
  ...
  const { tripDuration, budget } = location.state as {
    tripDuration: number;
    budget: string;
  };
    ...
    const fetchMatchingCities = async () => {
      try {
        const response = await axios.post<City[]>(
          "https://trippyee.onrender.com/api/match-cities",
          {
            tripDuration,
            budget,
          }
          ...
          <Card>
            <CardContent className="text-center py-8">
              <p className="text-xl text-gray-700">
                No matching cities found. Please try different criteria.
              </p>
              ...
              >
                ...
                <CardContent>
                  ...
                  <p className="text-gray-600 mb-4">
                    Ideal duration: {city.Min_no_of_days} -{" "}
                    {city.Max_no_of_days}
                  </p>
                  <p className="text-gray-700 mb-4 line-clamp-3">
                    {cleanDescription(city.City_desc)}
                  </p>
                  <Button
